...
args.save_path = 'experiments/' + args.dataset + '/dynamic_inference/channel_rdi/' + args.arch + str(args.depth) + '_wd' + str(args.weight_decay) + '_exec' + str(args.ratio) + '_r' + str(args.rscale) 
...
def train(trainloader, model, criterion, optimizer, epoch, use_cuda, args):
    ...
    for batch_idx, (inputs, targets) in enumerate(trainloader):
        ...
        progress_bar(batch_idx, len(trainloader), 'loss: %.2f | ce_loss: %.2f | Top1: %.2f | Top5: %.2f'
                    % (losses.avg, ce_losses.avg, top1.avg, top5.avg))
