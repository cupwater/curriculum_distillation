...
__all__ = ['resnet_channel_global']

class GatedConv_BN(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=1, ratio=1.0):
...
def conv3x3_BN(in_planes, out_planes, stride=1, ratio=1.0):
    return GatedConv_BN(in_planes, out_planes, kernel_size=3, stride=stride, padding=1, ratio=ratio)
    "3x3 convolution with padding"

class BasicBlock(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None, ratio=1.0):
        super(BasicBlock, self).__init__()
        self.conv1_bn = conv3x3_BN(inplanes, planes, stride, ratio=ratio)
        self.relu = nn.ReLU(inplace=True)
        self.conv2_bn = conv3x3_BN(planes, planes, ratio=ratio)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, gated=False, ratio=1.0):
        super(Bottleneck, self).__init__()
        self.conv1_bn = GatedConv_BN(inplanes, planes, kernel_size=1, bias=False, ratio=ratio)
        self.conv2_bn = GatedConv_BN(planes, planes, kernel_size=3, stride=stride,
                               padding=1, bias=False, ratio=ratio)
        self.conv3_bn = GatedConv_BN(planes, planes * 4, kernel_size=1, bias=False, ratio=ratio)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
        self.ratio = ratio
...
def resnet_channel_global(depth, num_classes=100, ratio=1.0):
    """
