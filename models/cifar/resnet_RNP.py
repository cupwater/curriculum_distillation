...
class BlockSkip(nn.Module):
    ...
    def forward(self, x, h, block_idx):
        ...
        #h = self.rnncell(self.relu(self.embeddings[block_idx](ss)), h)
...
class GatedConv_BN(nn.Module):
    ...
    def forward(self, x, ratios):
        ...
        #mask = torch.zeros(gate_out.size()).cuda()
        #topks = torch.round(ratios * self.conv.out_channels).type(torch.cuda.IntTensor)
        #sort_channels_idx = torch.argsort(gate_out, 1)
        #for i in range(x.size(0)):
        #    mask[i:(i+1),:].scatter_(1, sort_channels_idx[i, :topks[i]].view(1, -1), 1)
        #active_idx = (gate_out*mask).unsqueeze(2).unsqueeze(3)
        x = self.conv(x)
        x = self.bn(x)
        #x = x * active_idx
        return x
